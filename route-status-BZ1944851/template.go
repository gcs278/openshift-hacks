...
func (o *TemplateRouterOptions) Run(stopCh <-chan struct{}) error {
	...
	// Create shutdown handler to handle SIGTERM.
	// Important: Keep this handler call early or there will be gap in graceful shutdown handling.
	var wg sync.WaitGroup
	//go shutdownRouterHandler(&wg, &ptrTemplatePlugin, stopCh)
	...
	//wg.Wait()
	shutdownRouterHandler(&wg, &ptrTemplatePlugin, stopCh)

	return nil
}

// shutdownRouter ensures there is a proper shutdown of the router, providing a grace period and the
// proper shutdown function calls.
func shutdownRouterHandler(wg *sync.WaitGroup, templatePlugin **templateplugin.TemplatePlugin, stopCh <-chan struct{}) {
	// Add myself as a blocker for the wait group and remove me when done.
	wg.Add(1)
	defer wg.Done()

	// Block on stopCh until a signal (SIGTERM) is received.
	select {
	case <-stopCh:
		// 45s is the default interval that almost all cloud load balancers require to take an unhealthy
		// endpoint out of rotation.
		delay := getIntervalFromEnv("ROUTER_GRACEFUL_SHUTDOWN_DELAY", 1)
		...
		if templatePlugin != nil {
			...
			log.Info("Template router hasn't been created yet, exiting")
		}
