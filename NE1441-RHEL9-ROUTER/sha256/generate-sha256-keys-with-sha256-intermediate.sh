...
# intermediate
openssl req -newkey rsa:2048 -nodes -keyout example-inter.key -out example-inter.csr -subj '/CN=www.example.com/ST=SC/C=US/emailAddress=example@example.com/O=Example/OU=Example'
openssl req -x509 -sha256 -newkey rsa:2048 -days 3650 -in example-inter.csr -CA exampleca.crt -CAkey exampleca.key -keyout example-inter.key -out example-inter.crt -addext "keyUsage=cRLSign, digitalSignature, keyCertSign" -addext "extendedKeyUsage=serverAuth, clientAuth" -nodes -subj '/C=US/ST=SC/L=Default City/O=Default Company Ltd/OU=Test CA/CN=www.exampleinter.com/emailAddress=example@example.com'
